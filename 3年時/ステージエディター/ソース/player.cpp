...
HRESULT CPlayer::Init(float fPosX, float fPosY, float fPosZ, unsigned int nModelNum, unsigned int nTexNum)
{
	...
	m_rot.y = CManager::GetCamera()->GetRot();							// プレイヤーの向きを設定	
...
//if( CManager::GetFade()->GetFadeMode() == CFade::FADEMODE_NONE )
	//{
		...
		{	
			m_rot.y = CManager::GetCamera()->GetRot();
		...
		{
			m_rot.y = CManager::GetCamera()->GetRot() + D3DX_PI;
		...
		{
			m_rot.y = CManager::GetCamera()->GetRot() + D3DX_PI + D3DX_PI / 2;
			m_pos.x += cosf(m_rot.y + D3DX_PI / 2) * MOVE_X;
			m_pos.z -= sinf(CManager::GetCamera()->GetRot()) * MOVE_X;
			m_bMove = true;
		}
		else if(pKeyboard->KeyState(DIK_D))
		{
			m_rot.y = CManager::GetCamera()->GetRot() + D3DX_PI / 2;
			m_pos.x -= cosf(m_rot.y - D3DX_PI / 2) * MOVE_X;
		...
		{
			// 地面が取得できたのなら
			fHeight = pField->GetHeight(m_pos);	// 地面の高さを取得
		}
